...
| `--data-dir string`   | The directory for the Lerax Chain data if the local FS is used   | ""            | YES       |
...
- `--config` and `--data-dir`: These flags are mutually exclusive. Use either `--config` to specify the path to the SecretsManager config file or `--data-dir` to set the directory for the Lerax Chain data if the local FS is used.
...
| `--data-dir` string | The data directory used for storing Lerax Chain client data. | “” | YES | Command: server Flag:--data-dir “./test-chain-1” | NO |
